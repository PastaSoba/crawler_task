...
class Illusie300:
    baseurl = "http://mb.palemo.com/brand.php?brand=11&id=illusie300"
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=Illusie300._result_df.columns)
        Illusie300._result_df = Illusie300._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        page = Illusie300._OpenURLWithBeautifulSoup(Illusie300.baseurl)

        stores = page.find("div", {"id": "ShopList"}).find(
            "div", {"class": "ContBox"}).find_all("tr")

        for store in stores:
            storeInfo = store.find_all("td")
            storeName = storeInfo[0].find("a").text
            storeAddress = storeInfo[1].text

            storeName = "illusie300 " + storeName
            storeAddress = re.sub(".*【 MAP 】", "", storeAddress)

            Illusie300._AppendItemToDataFrame(storeName, storeAddress)

        return Illusie300._result_df


if __name__ == "__main__":
    df = Illusie300.getStoreInfo()
