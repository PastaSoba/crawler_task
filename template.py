...
class TEMPLATE:
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=TEMPLATE._result_df.columns)
        TEMPLATE._result_df = TEMPLATE._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        page = TEMPLATE._OpenURLWithBeautifulSoup(TEMPLATE.baseurl)

        # # 実行確認用
        # print(TEMPLATE._result_df)
        # TEMPLATE._result_df.to_csv('./csv/TEMPLATE.csv')
        return TEMPLATE._result_df


if __name__ == "__main__":
    df = TEMPLATE.getStoreInfo()
