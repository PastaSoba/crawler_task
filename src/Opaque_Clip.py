...
class Opaque_Clip:
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=Opaque_Clip._result_df.columns)
        Opaque_Clip._result_df = Opaque_Clip._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        BASE_URL = 'https://store.world.co.jp/real-store-search?pref=&br=BR637&pcf=1&so=1%2C3&page='
        PAGE_RANGE = 3  # page=1からpage=3まである

        for page_index in range(1, PAGE_RANGE + 1):
            URL = BASE_URL + str(page_index)
            page = Opaque_Clip._OpenURLWithBeautifulSoup(URL)
            storelist_lis = page.find_all('li', class_="point")
            for store in storelist_lis:
                storeName = store.find('a').text.replace(
                    '\n', '').replace("OPAQUE.CLIP", "OPAQUE.CLIP ").strip()
                storeAddress = store.find(
                    'div', class_="txt_address").text.split('\n')[1].strip()
                Opaque_Clip._AppendItemToDataFrame(storeName, storeAddress)

        # 実行確認用
        # print(Opaque_Clip._result_df)
        # Opaque_Clip._result_df.to_csv('./Opaque_Clip.csv')
        return Opaque_Clip._result_df


if __name__ == "__main__":
    df = Opaque_Clip.getStoreInfo()
