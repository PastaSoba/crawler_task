...
class Rageblue:
    baseurl = "https://www.adastria.co.jp/shop/result.html?brand_id%5B%5D=107"
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=Rageblue._result_df.columns)
        Rageblue._result_df = Rageblue._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        page = Rageblue._OpenURLWithBeautifulSoup(Rageblue.baseurl)
        ...
        for store in stores:
            ...
            Rageblue._AppendItemToDataFrame(storeName, storeAddress)

        # # 実行確認用
        # print(Rageblue._result_df)
        Rageblue._result_df.to_csv('./Rageblue.csv')
        return Rageblue._result_df


if __name__ == "__main__":
    df = Rageblue.getStoreInfo()
