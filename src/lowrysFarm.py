...
class lowrysFarm:
    baseurl = "https://www.adastria.co.jp/shop/result.html?brand_id%5B%5D=101&area_id%5B%5D="
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=lowrysFarm._result_df.columns)
        lowrysFarm._result_df = lowrysFarm._result_df.append(
    ...
    def getStoreInfo():
        ...
        for pageid in range(1, pagenum + 1):
            page = lowrysFarm._OpenURLWithBeautifulSoup(
                lowrysFarm.baseurl + str(pageid))
            prefs = page.find_all("table", class_="shop-search-result-table")
            stores = [store for pref in prefs for store in pref.find_all("tr")]
            for store in stores:
                storeName = store.find("td", class_="s01").find("a").text
                storeName = re.sub('[\r\n]+', '', storeName)
                storeAddress = store.find("td", class_="s02").text
                lowrysFarm._AppendItemToDataFrame(storeName, storeAddress)

        # # 実行確認用
        # print(lowrysFarm._result_df)
        # lowrysFarm._result_df.to_csv('./lowrysFarm.csv')
        return lowrysFarm._result_df


if __name__ == "__main__":
    df = lowrysFarm.getStoreInfo()
