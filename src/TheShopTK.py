...
class TheShopTK:
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=TheShopTK._result_df.columns)
        TheShopTK._result_df = TheShopTK._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        BASE_URL = 'https://store.world.co.jp/real-store-search?pref=&br=BR616&pcf=1&so=1%2C3&page='
        PAGE_RANGE = 5  # page=1からpage=5まである

        for page_index in range(1, PAGE_RANGE + 1):
            URL = BASE_URL + str(page_index)
            page = TheShopTK._OpenURLWithBeautifulSoup(URL)
            storelist_lis = page.find_all('li', class_="point")
            for store in storelist_lis:
                true_name = "THE SHOP TK"
                storeName = store.find('a').text.replace(
                    '\n', '').replace("ＴＨＥ ＳＨＯＰ ＴＫ", true_name).replace("ＴＨＥ　ＳＨＯＰ　ＴＫ", true_name).replace(true_name, "THE SHOP TK ").strip()
                storeAddress = store.find(
                    'div', class_="txt_address").text.split('\n')[1].strip()
                TheShopTK._AppendItemToDataFrame(storeName, storeAddress)

        # 実行確認用
        print(TheShopTK._result_df)
        TheShopTK._result_df.to_csv('./TheShopTK.csv')
        return TheShopTK._result_df


if __name__ == "__main__":
    df = TheShopTK.getStoreInfo()
