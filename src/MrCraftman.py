...
class MrCraftman:
    baseurl = "https://www.riat-rs.com/shop/find.jsp?pref={}"
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=MrCraftman._result_df.columns)
        MrCraftman._result_df = MrCraftman._result_df.append(
    ...
    def getStoreInfo():
        ...
        for index in range(1, page_num + 1):
            try:
                page = MrCraftman._OpenURLWithBeautifulSoup(
                    MrCraftman.baseurl.format(index))
                ...
                for store in stores:
                    ...
                    if "ミスタークラフトマン" in storeName:
                        MrCraftman._AppendItemToDataFrame(
                            storeName, storeAddress)
            except Exception:
                pass

        return MrCraftman._result_df


if __name__ == "__main__":
    df = MrCraftman.getStoreInfo()
