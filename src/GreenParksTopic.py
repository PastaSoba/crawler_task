...
class GreenParksTopic:
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=GreenParksTopic._result_df.columns)
        GreenParksTopic._result_df = GreenParksTopic._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        page = GreenParksTopic._OpenURLWithBeautifulSoup(
            GreenParksTopic.baseurl)
        ...
        for store in stores:
            ...
            if "topic" in storeName:
                GreenParksTopic._AppendItemToDataFrame(storeName, storeAddress)

            if storeName == "Festival Walk i.t":
                break
            # # 実行確認用
            # print(GreenParksTopic._result_df)
            # GreenParksTopic._result_df.to_csv('./GreenParksTopic.csv')
        return GreenParksTopic._result_df


if __name__ == "__main__":
    df = GreenParksTopic.getStoreInfo()
