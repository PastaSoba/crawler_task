...
class heartMarket:
    baseurl = "http://www.burantasu.com/shoplist/I/175/d-all.html"
    ...
    def _AppendItemToDataFrame(storeName: str, storeAddress: str):
        ...
        tmp_se = pd.Series([storeName, storeAddress],
                           index=heartMarket._result_df.columns)
        heartMarket._result_df = heartMarket._result_df.append(
    ...
    @staticmethod
    def getStoreInfo():
        page = heartMarket._OpenURLWithBeautifulSoup(heartMarket.baseurl)
        stores = page.find("table", class_="tbl-shop").find_all("tr")
        for store in stores:
            ...
            heartMarket._AppendItemToDataFrame(storeName, storeAddress)

        # # 実行確認用
        # print(heartMarket._result_df)
        # heartMarket._result_df.to_csv('./heartMarket.csv')
        return heartMarket._result_df


if __name__ == "__main__":
    df = heartMarket.getStoreInfo()
